    ...
    for (const card of jobCards) {
      try {
        ...
        const jobData = await page.evaluate(() => {
          // Try to read from JSON-LD structured data first
          // This is the cleanest source of job information
          ...
          if (jsonLdScript) {
            try {
              ...
              // JSON parsing failed, fall through to CSS selectors
            }
          }

          // Fallback to CSS selectors if JSON-LD not available
          const getText = (selector: string) =>
            document.querySelector(selector)?.textContent?.trim() || ""

          const title = getText("h1.text-center") || getText("h1")

          const description = getText("div.job-description") +
            "\n\n" + getText("div.job-qualifications")

          const location = document.querySelector(
            "li.withicon.location-dot span"
          )?.textContent?.trim() || "Tanzania"

          const jobType = document.querySelector(
            "li.withicon.file-signature span"
          )?.textContent?.trim() || null

          // Try to find salary
          let salary = null
          document.querySelectorAll("ul.arrow-list li").forEach(li => {
            if (li.textContent?.includes("Salary")) {
              salary = li.querySelector("span")?.textContent?.trim() || null
            }
          })

          return { title, description, location, jobType, salary, company: "", validThrough: null }
        })
        ...
        // Use company from card if not found on page
        ...
        jobsAdded++
        console.log(`Added: ${jobData.title} at ${company} in ${jobData.location}`)
